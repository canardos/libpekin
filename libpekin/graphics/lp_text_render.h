...
public:
     ...
     * @param y_pos y position of top left corner. May be offscreen
     * @param font 1/2/4/8 bpp supported
     * @param character ASCII character code
     * @param color array of 2^bpp color elements
     *
     * @return # x pixels to advance for next letter or 0 on error
     */
    template <RasterFontConcept Font>
    uint16_t drawChar(uint16_t x_pos, int16_t y_pos, const Font& font, uint8_t character, const T color[])
    {
    ...
    uint16_t drawText(
            int16_t x, int16_t y,
            const Font& font, const char* text, Align align, const T color[],
            bool no_y_offs = false)
    {
        // TODO: update interface to return cursor x pos vs width
        // TODO: check color len against font bpp
        // TODO: if no_y_offset and y is middle
        ...
        uint16_t i = 0;
        if (no_y_offs) {
            int16_t y_offset = 0x7fff;
            // Determine smallest y offset and shift text up by this amount
            while (text[i] != 0) {
                y_offset = std::min(y_offset, font.charMeta(text[i++]).offs.y);
            }
            y -= y_offset;
        }
        i = 0;
    ...
    static Rect16 getTextBounds(const Font& font, const char* text) {
        ...
        while (text[i] != 0) {
            ...
            i++;
            // Use width, not x-advance on last char
            width += text[i] == 0 ? meta.size.x : meta.x_adv;
            max_height = std::max(max_height, meta.size.y);
        }
        // TODO: document limits
        Rect16 r = { width, static_cast<uint16_t>(max_height) };
        return r;
    }
